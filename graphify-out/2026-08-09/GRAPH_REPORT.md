...
## Corpus Check
- 182 files · ~43,248 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1130 nodes · 2029 edges · 75 communities (43 shown, 32 thin omitted)
...
1. `Warehouse.Domain` - 51 edges
2. `Warehouse.Application.DTOs` - 46 edges
3. `Warehouse.Infrastructure.Migrations` - 39 edges
4. `Container` - 33 edges
5. `Result` - 31 edges
6. `Warehouse.Application.Interfaces` - 30 edges
7. `PickTask` - 29 edges
8. `Location` - 25 edges
9. `PutawayTask` - 25 edges
...
Cohesion: 0.07
Nodes (31): AllowAnonymous, ActionResult, ResultExtensions, ActionResult, Authorize, Guid, HttpGet, HttpPost (+23 more)
...
Cohesion: 0.20
Nodes (5): EntityTypeBuilder, PutawayTaskConfiguration, EntityTypeBuilder, XminConcurrencyExtensions, Warehouse.Infrastructure.Configurations

### Community 41 - "Container"
Cohesion: 0.28
Nodes (6): DateTime, Guid, StockTransaction, StockTransactionType, EntityTypeBuilder, StockTransactionConfiguration
...
Cohesion: 0.08
Nodes (16): SplitAndCloseDto, IStockTransactionRepository, IConfiguration, DependencyInjection, AppDbContext, StockTransactionRepository, Warehouse.Api.Common, Warehouse.Application.Common (+8 more)
...
Cohesion: 0.07
Nodes (31): ActionResult, Guid, HttpGet, HttpPost, IActionResult, IEnumerable, List, Task (+23 more)
...
## Knowledge Gaps
- **166 isolated node(s):** `$schema`, `commandName`, `dotnetRunMessages`, `launchBrowser`, `applicationUrl` (+161 more)
...
- **Why does `Warehouse.Domain` connect `Warehouse.Api.Common` to `RegisterDto.cs`, `StockConfiguration`, `Orders API & Allocation`, `Snapshot: InitialCreate`, `Containers API & Mapping`, `Stock`, `Snapshot: ProductVolumetrics1`, `Container`, `PutawayTask`, `AppDbContext`, `IEntityTypeConfiguration`, `PickTask Entity Mapping`, `PickTaskItem`, `PutawayTaskItem`, `Stock`?**
  _High betweenness centrality (0.107) - this node is a cross-community bridge._
- **Why does `AppDbContext` connect `AppDbContext` to `RegisterDto.cs`, `Orders API & Allocation`, `Containers API & Mapping`, `Stock`, `Snapshot: InitialCreate`, `Container`, `PutawayTask`, `PickTask Entity Mapping`, `PickTaskItem`, `PutawayTaskItem`, `Stock`?**
